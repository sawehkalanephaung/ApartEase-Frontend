...
const fetchData = async () => {
  try {
    console.log('Fetching data for unit history ID:', route.params.id);
    ...
    if (response.data && response.data.BillHistoryDetail) {
      ...
      rentCostInput.value = unitData.rentCost || rentCostInput.value;
      status.value = unitData.approveStatus || status.value;
      roomNumber.value = unitData.res_room || roomNumber.value;
      residentName.value = unitData.residentName || residentName.value;
      residentEmail.value = unitData.residentEmail || residentEmail.value;
    } else {
      console.warn('No unit history data found in response:', response.data);
    }
  } catch (error) {
